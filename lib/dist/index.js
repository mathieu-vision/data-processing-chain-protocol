...
var MonitoringAgent = class _MonitoringAgent extends Agent {
  ...
  constructor() {
    super();
    this.chainHierarchy = /* @__PURE__ */ new Map();
    this.workflow = {};
  ...
  genReportingAgent(payload) {
    ...
    reporting.on("global-signal", (signal) => __async(this, null, function* () {
      ...
      if (index > 0) {
        ...
        console.log("REPORTING CALLBACK CALLED");
...
`
  ...
  //
  handleChildChainCompletion(childChainId) {
    return __async(this, null, function* () {
      ...
      if (parentEntry) {
        ...
        yield this.checkChainReadiness(childEntry.parentId);
      }
    });
  }
  trackChildChain(parentChainId, childChainId) {
  ...
  checkChainReadiness(chainId) {
    return __async(this, null, function* () {
      try {
        ...
        if (setupCount >= config && entry.children.length === entry.completedChildren.size) {
          try {
            ...
            Logger.error(
              `Failed to send start signal for chain ${chainId}: ${error.message}`
            );
            throw error;
          }
        }
        ...
        Logger.error(
          `Error during chain readiness check for ${chainId}: ${error.message}`
        );
        throw error;
      }
    });
...
var Node = class _Node {
  ...
  *getPipelineGenerator(pipelines, count) {
  ...
  processChildChain(data) {
    return __async(this, null, function* () {
      var _a;
      if ((_a = this.config) == null ? void 0 : _a.chainConfig) {
        const supervisor = NodeSupervisor.retrieveService();
...
var NodeSupervisor = class _NodeSupervisor {
  ...
  createChain(config) {
    ...
    try {
      ...
      Logger.header(`${this.ctn}{createChain}: ${error.message}`);
      throw error;
    }
  }
  /**
      ...
      return __async(this, null, function* () {
        ...
        switch (status) {
          ...
          case ChainStatus.CHILD_CHAIN_COMPLETED:
            yield monitoring.handleChildChainCompletion(
              ""
            );
            break;
          /*
