...
var ChainStatus;
((ChainStatus2) => (ChainStatus2.NODE_PENDING = "node_pending", ChainStatus2.NODE_IN_PROGRESS = "node_in_progress", ChainStatus2.NODE_COMPLETED = "node_completed", ChainStatus2.NODE_FAILED = "node_failed", ChainStatus2.NODE_SETUP_COMPLETED = "node_setup_completed", ChainStatus2.CHAIN_DEPLOYED = "chain_deployed", ChainStatus2.CHAIN_SETUP_COMPLETED = "chain_setup_completed", ChainStatus2.CHILD_CHAIN_STARTED = "child_chain_started", ChainStatus2.CHILD_CHAIN_COMPLETED = "child_chain_completed", ChainStatus2.NODE_PENDING_DELETION = "node_pending_deletion", ChainStatus2.NODE_END_OF_PIPELINE = "node_end_of_pipeline", ChainStatus2.NODE_SUSPENDED = "node_suspended"))(ChainStatus || (ChainStatus = {}));
var NodeSignal;
...
var _ReportingAgentBase = class _ReportingAgentBase extends Agent {
  ...
  notify(notificationStatus, type = "local-signal") {
    ...
    Logger.info(`Status ${status} from ${this.uid}`), this.status.push(status), this.emit(type, notificationStatus);
  }
  /**
  ...
  notify(status, type = "local-signal") {
    try {
      if (this.reporting !== null)
        typeof status == "object" && "status" in status ? this.reporting.notify(status, type) : this.reporting.notify({ status }, type);
      else
...
var NodeSupervisor = class _NodeSupervisor {
  ...
  enqueueSignals(nodeId, status) {
    var _a;
    (_a = this.nodes.get(nodeId)) == null || _a.enqueueSignals(status);
  }
  fallbackSignalsQueue(message) {
  }
  /**
  ...
  handleNotification(chainId, status) {
    try {
      ...
      node.notify(status, "global-signal"), Logger.info(
        `${this.ctn}:
...
((Ext6) => {
  ...
  Ext6.defaultMonitoringResolver = (chainId) => __async(void 0, null, function* () {
    try {
      ...
      throw new Error("Monitoring host not found");
    } catch (error) {
