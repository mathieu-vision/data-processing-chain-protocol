...
// src/utils/Logger.ts
var import_fs = require("fs"), import_path = require("path"), import_util = require("util");
var Colors = {
  reset: "\x1B[0m",
  info: "\x1B[32m",
  // green
...
`;
  ...
  static special(message) {
    let msg = typeof message == "string" ? message : (0, import_util.format)(message);
...
var DEFAULT_LOG_PATH = (0, import_path.join)(process.cwd(), "logs"), logStream, getLogFileName = () => `dpcp-${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}.log`, initDiskLogger = () => {
  try {
    (0, import_fs.mkdirSync)(DEFAULT_LOG_PATH, { recursive: !0 });
    let logFile = (0, import_path.join)(DEFAULT_LOG_PATH, getLogFileName());
    return logStream = (0, import_fs.createWriteStream)(logFile, { flags: "a" }), !0;
  } catch (err) {
...
var NodeSignal;
((NodeSignal2) => (NodeSignal2.NODE_SETUP = "node_setup", NodeSignal2.NODE_CREATE = "node_create", NodeSignal2.NODE_DELETE = "node_delete", NodeSignal2.NODE_RUN = "node_run", NodeSignal2.NODE_SEND_DATA = "node_send_data", NodeSignal2.NODE_ERROR = "node_error", NodeSignal2.NODE_RESUME = "node_resume", NodeSignal2.NODE_STOP = "node_stop", NodeSignal2.NODE_SUSPEND = "node_suspend", NodeSignal2.CHAIN_PREPARE = "chain_prepare", NodeSignal2.CHAIN_START = "chain_start", NodeSignal2.CHAIN_START_PENDING_OCCURRENCE = "chain_start_pending_occurrence", NodeSignal2.CHAIN_DEPLOY = "chain_deploy"))(NodeSignal || (NodeSignal = {}));

// src/core/Node.ts
var import_timers = require("timers"), import_node_crypto2 = require("crypto");

// src/agents/Agent.ts
var import_node_events = __toESM(require("events")), import_node_crypto = require("crypto"), Agent = class extends import_node_events.default {
  /**
   * Creates a new Agent instance with a unique identifier
   */
  constructor() {
    super(), this.uid = (0, import_node_crypto.randomUUID)();
  }
};
...
var Node = class _Node {
  ...
  constructor(dependencies = []) {
    this.reporting = null;
    this.id = (0, import_node_crypto2.randomUUID)(), this.output = [], this.pipelines = [], this.dependencies = dependencies, this.status = ChainStatus.NODE_PENDING, this.progress = 0, this.dataType = DataType.RAW, this.executionQueue = Promise.resolve(), this.nextNodeInfo = null, this.config = null, this.statusManager = new NodeStatusManager(this);
  }
  /**
...
var NodeSupervisor = class _NodeSupervisor {
  ...
  createChain(config) {
    ...
    try {
      ...
      let timestamp = Date.now(), chainId = `${this.uid}-${timestamp}-${(0, import_node_crypto3.randomUUID)().slice(0, 8)}`, relation = {
        config
      };
      ...
      return __async(this, null, function* () {
        ...
        switch (status) {
          ...
          case ChainStatus.CHAIN_NOTIFIED: {
            let { signal, payload } = message.signal, supervisor = NodeSupervisor.retrieveService(), nodeStatusMessage = {
              payload,
              chainId,
              signal: signal != null ? signal : NodeSignal.NODE_ERROR,
              nodeId: "",
              index: 0,
              count: 0
            };
...
// src/utils/http.ts
var import_buffer = require("buffer"), http = __toESM(require("http")), https = __toESM(require("https"));
...
((Ext6) => {
  ...
  let broadcastReportingCallback = (payload) => __async(void 0, null, function* () {
    try {
      ...
      Logger.info(`BroadcastReportingCallback: Sending message to ${url}`), yield post(url, data);
    } catch (error) {
      Logger.error({ message: error.message });
    }
  });
...
var Ext4;
((Ext6) => {
  Ext6.defaultHostResolver = (message) => __async(void 0, null, function* () {
    var _a;
    try {
      let hostURI = (_a = message.payload) == null ? void 0 : _a.hostURI;
      if (hostURI)
        return hostURI;
  ...
  let nodeStatusCallback = (payload) => __async(void 0, null, function* () {
    try {
      let { message, path, hostResolver } = payload, host = yield hostResolver(message), url = new URL(path, host), data = JSON.stringify(message);
  ...
  Ext6.setNodeStatusResolverCallbacks = (dcPayload) => __async(void 0, null, function* () {
    let { paths, hostResolver } = dcPayload;
    NodeSupervisor.retrieveService().setNodeStatusCallback(
      (message) => __async(void 0, null, function* () {
        let payload = {
          message,
          hostResolver: hostResolver != null ? hostResolver : Ext6.defaultHostResolver,
          path: paths.enqueue
        };
