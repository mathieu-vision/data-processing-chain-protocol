...
// src/core/NodeReporter.ts
var _NodeReporter = class _NodeReporter {
  constructor() {
    if (!(_NodeReporter.authorizedController instanceof NodeController)) {
      throw new Error(
        "Node Reporter needs to be instantiated by a Node Controller"
      );
    }
    _NodeReporter.authorizedController = null;
  }
  static authorize(controller) {
    _NodeReporter.authorizedController = controller;
  }
};
_NodeReporter.authorizedController = null;
var NodeReporter = _NodeReporter;

// src/core/NodeController.ts
var NodeReporterAgent = class extends NodeReporter {
};
var NodeController = class _NodeController {
  constructor() {
  }
  static retrieveService(refresh = false) {
    if (!_NodeController.instance || refresh) {
      const instance = new _NodeController();
      _NodeController.instance = instance;
    }
    return _NodeController.instance;
  }
  getReporterAgent() {
    NodeReporterAgent.authorize(this);
    const reporter = new NodeReporterAgent();
    return reporter;
  }
...
var Node = class _Node {
  constructor(dependencies = []) {
    ...
    this.config = null;
    const controller = NodeController.retrieveService();
    this.reporter = controller.getReporterAgent();
  }
  updateProgress() {
